...
local function tokenize_html(html)
    ...
    while true do
        ...
        if not stop then
            ...
            table.insert(tokens, {
                type = "text",
                text = html:sub(start, start)
            })
            pos = start + 1
        else
            table.insert(tokens, {
                type = "tag",
                text = html:sub(start, stop)
            })
            pos = stop + 1
        end
    end
    ...
    local function find_list(array, sublist)
        ...
        local function find_list_end(array, start)
            ...
            for i = start, #array - 1 do
                if array[i].type == "blank" and array[i + 1].type ~= "list_item" and array[i + 1].type ~= "indented" and
                    array[i + 1].type ~= "blank" then
                    pos = i - 1
                    break
                end
            end
...
local function codeblocks(lines)
    local function find_codeblock(lines)
        local start
        for i, line in ipairs(lines) do
            if line.type == "indented" or line.type == "codeblock" then
                start = i
                break
            end
        end
...
local function footnotes(text)
    return text:gsub("%[%^(%d+)%]: ([^\n]+)",
        "<div class=\"footnote\" id=\"fn-%1\"><a href=\"#ref-%1\">%1</a>. %2</div>"):gsub("%[%^(%d+)%]",
        "<a href=\"#fn-%1\" id=\"ref-%1\">[%1]</a>")
end
